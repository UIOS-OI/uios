...
export function GET(request: NextRequest) {
  ...
  return Response.json({ tenantId, records: query ? memoryStore.search(tenantId, query) : memoryStore.list(tenantId) });
}
...
export async function POST(request: NextRequest) {
  ...
  return Response.json({ record: memoryStore.save(tenantId, body.content.trim(), metadata) }, { status: 201 });
}
