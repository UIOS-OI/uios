...
export async function POST(request: NextRequest) {
  ...
  const tenantId = resolveTenantId(request); const rate = checkRateLimit(tenantId, "approval"); if (!rate.allowed) return Response.json({ error: "Approval rate limit reached.", retryAfterSeconds: rate.retryAfterSeconds }, { status: 429, headers: { "Retry-After": String(rate.retryAfterSeconds) } });
  ...
  analytics.track(tenantId, "workflow.approval.issued", { workflowId, nodeCount: nodeIds.length, expiresInSeconds: 300 });
