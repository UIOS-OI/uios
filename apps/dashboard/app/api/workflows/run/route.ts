...
export async function POST(request: NextRequest) {
  ...
  if (approvalNodes.length > 0 && (!body.approvalToken || !verifyWorkflowApproval(body.approvalToken, tenantId, workflow.id, approvalNodes))) {
    await analytics.track(tenantId, "workflow.approval.required", { workflowId: workflow.id, nodeCount: approvalNodes.length, tokenPresented: Boolean(body.approvalToken) });
  ...
  if ((await getUsage(tenantId)).units + units > planLimit) return Response.json({ error: "UIOS usage limit reached." }, { status: 402 });
  ...
  if (run.status === "completed") await recordUsage(tenantId, units, "workflow_run");
  await analytics.track(tenantId, "workflow.run", { workflowId: workflow.id, status: run.status, nodes: workflow.nodes.length, modelNodes });
