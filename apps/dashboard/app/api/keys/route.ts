...
export function GET(request: NextRequest) {
  ...
  return Response.json({ tenantId, keys: listApiKeys(tenantId) });
}
...
export async function POST(request: NextRequest) {
  ...
  if (role === "owner" && resolveAuth(request).role !== "owner") return Response.json({ error: "Only an owner can create another owner key." }, { status: 403 });
  const rawKey = `uios_live_${randomBytes(24).toString("base64url")}`;
  const record = saveApiKey({ id: `key_${randomUUID().replace(/-/g, "")}`, tenantId, name, role, keyPrefix: rawKey.slice(0, 18), keyHash: hashApiKey(rawKey), createdAt: new Date().toISOString() });
...
export async function DELETE(request: NextRequest) {
  ...
  const targetRole = getApiKeyRole(tenantId, id);
  if (!targetRole) return Response.json({ error: "Key was not found or is already revoked." }, { status: 404 });
  if (targetRole === "owner" && resolveAuth(request).role !== "owner") return Response.json({ error: "Only an owner can revoke an owner key." }, { status: 403 });
  if (!revokeApiKey(tenantId, id)) return Response.json({ error: "Key was not found or is already revoked." }, { status: 404 });
