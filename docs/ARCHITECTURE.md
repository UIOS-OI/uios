...
Planetary appearance is deterministic and topology-driven. Each planet ID seeds procedural terrain, ocean/rock balance, cloud cover, polar ice, atmospheric scattering, gas-band behavior, and optional rings. New category, provider, and feature nodes therefore inherit a distinct astronomical body without adding texture assets or bespoke component branches. The background similarly combines two stellar depth bands, varied star temperatures, slow twinkle, a galactic dust plane, and region-specific nebular color.

Nested host planets add a second deterministic identity layer inspired by luminous geometric orbs: each ID selects a crystalline polyhedron, concentric mandala, cellular geodesic lattice, or orbital-flower shell. The shell surrounds the smaller physical planet core, rotates independently, breathes at a seeded phase, and uses additive materials compatible with the existing bloom pipeline. This is generated geometry rather than a bitmap texture, so provider and feature planets inherit the same visual grammar automatically.
...
Memory artifacts preserve their allowlisted repository path but are hosted beneath a department planet instead of being flattened directly into the Memory system. A deterministic keyword router places governance and security material under Policies; architecture and provider material under Engineering; commercial material under Sales or Finance; identity material under HR; and user-facing material under Customer. Unmatched technical files safely default to Engineering. Entering a host planet streams its authored world structures and file gems together.
